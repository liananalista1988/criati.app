...
- Conceito novo: `Aplicacao` (tabela `aplicacao`, código único, nome, descrição, status) é o catálogo global de soluções que a Criati oferece — hoje `FINANCEIRO` e `TAREFAS_PROCESSOS` (operacionais) e `CLINICA` (demonstração), criado por `V4__criar_estrutura_aplicacoes.sql` e complementado pelo seed tipado. Uma aplicação inativa nunca pode ser utilizada por nenhuma empresa, mesmo que o vínculo esteja ativo.
...
- `AplicacaoCatalogoSeedRunner` (`ApplicationRunner`, roda em todo perfil) garante que o catálogo tipado (`FINANCEIRO`, `CLINICA`, `TAREFAS_PROCESSOS`) exista de forma idempotente, complementando `V4__criar_estrutura_aplicacoes.sql`: em Postgres (local/homolog/prod) a migration já insere os dois primeiros e o runner inclui Trabalho sem alteração estrutural; no perfil `test`, o Flyway está desabilitado (`ddl-auto=create-drop`, ver `application-test.properties`) e o runner semeia todo o catálogo. Módulos marcados como indisponíveis, como `ESTOQUE`, não são persistidos.
...
- Tarefas e Processos integra o catálogo como `TAREFAS_PROCESSOS`, mas preserva `PaginaTrabalhoController` como controlador exclusivo de suas páginas. Página e API exigem contexto válido e vínculo técnico ativo do módulo; `ADMINISTRADOR` mantém operações estruturais e o responsável mantém somente as transições autorizadas de sua própria tarefa. O menu é preparado pelo serviço compartilhado de navegação e não decide disponibilidade no JavaScript.
